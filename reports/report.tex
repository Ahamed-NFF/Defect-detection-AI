...
% University logo. Save the Ruhuna crest as reports/figures/uor_logo.jpg;
% until then a placeholder box is shown so the document still compiles.
\IfFileExists{figures/uor_logo.jpg}
  {\includegraphics[width=0.20\textwidth]{figures/uor_logo.jpg}}
...
\multirow{3}{*}{hazelnut}
 ...
 & Ours       & 1.000 & 1.000 & \textbf{1.000} & 1.000 & 1.000 \\
\midrule
\textit{all} & Combined (ours) & 1.000 & 0.971 & 0.986 & 0.991 & 1.000 \\
\bottomrule
\end{tabular}
\end{table}

A single classifier trained jointly over all three categories
(\textit{Combined}, last row) reaches defect-F1~0.986 / AUROC~1.0 on the pooled
180-image test set---within a point of the per-category models. This shows the
method yields \emph{one deployable inspector} that handles every product type,
rather than three separate models, which is how the demo tool is served.
...
\subsection{Prompt engineering for inspector descriptions}
Beyond detection, the tool generates natural-language descriptions of flagged
defects using LLaVA, served locally via Ollama. To make this a systematic study
rather than a single ad-hoc prompt, we compare four prompt variants that share a
common inspector system prompt: \emph{zero-shot} (a plain instruction);
\emph{chain-of-thought} (CoT: identify type $\rightarrow$ localise
$\rightarrow$ rate severity $\rightarrow$ recommend an action); \emph{few-shot}
(two worked image\,+\,answer exemplars supplied as prior turns); and their
\emph{combination} (CoT~+~few-shot). Each variant is run over a fixed set of
held-out defect images, and the outputs are scored by a human rater on a 1--5
scale for defect-type accuracy, localisation accuracy, and severity accuracy
(Table~\ref{tab:prompts}). Structured prompting (CoT, and especially CoT~+~few-shot)
is expected to yield more consistent, inspection-ready descriptions than the
zero-shot baseline.

\begin{table}[h]
\centering
\caption{Prompt-variant comparison (human-rated, 1--5; mean over the 15-image
evaluation set, 5 per category). Higher is better. Produced by
\texttt{scripts/run\_prompt\_eval.py}.}
\label{tab:prompts}
\begin{tabular}{lcccc}
\toprule
Variant & Type & Location & Severity & Overall \\
\midrule
Zero-shot              & 2.0 & 2.5 & 1.5 & 2.0 \\
Chain-of-thought (CoT) & 3.0 & 3.5 & 4.0 & 3.5 \\
Few-shot               & 3.0 & 3.0 & 3.0 & 3.0 \\
\textbf{CoT + few-shot}& \textbf{3.5} & \textbf{4.0} & \textbf{4.0} & \textbf{4.0} \\
\bottomrule
\end{tabular}
\end{table}

Structured prompting clearly outperformed the zero-shot baseline. The zero-shot
prompt frequently \emph{denied} the defect---rationalising it as natural
variation (e.g.\ ``a natural occurrence in the formation of the shell'') or
reporting ``no visible defects''---and seldom produced a severity rating or an
action. Chain-of-thought reliably emitted the four requested fields with
sensible severity reasoning, and the combined CoT~+~few-shot variant was the most
consistent at producing compact, inspection-ready descriptions in the exemplar
format. A cross-cutting limitation of the 7B model was occasional \emph{object}
misidentification (describing a hazelnut as an ``acorn'', or a bottle as a
``lens cap''), although the reported defect attributes were usually still
reasonable; a larger model (e.g.\ llava:13b) would likely reduce this. The
contrast is stark on the same hazelnut image: zero-shot concluded it was ``a
natural defect that does not affect the edible part of the nut'', whereas
CoT~+~few-shot returned ``Defect type: crack. Location: visible on the part
surface. Severity: moderate to severe. Recommended action: reject the unit and
